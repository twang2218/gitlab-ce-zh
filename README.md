...
- [`10.4`, `10.4.0`, `latest` (*10.4/Dockerfile*)](https://github.com/twang2218/gitlab-ce-zh/blob/master/10.4/Dockerfile)
...
```bash
docker run -d -p 3000:80 twang2218/gitlab-ce-zh:10.4.0
```
...
services:
    gitlab:
      image: 'twang2218/gitlab-ce-zh:10.4.0'
...
这表明将使用 `gitlab/gitlab-ce:10.4.0-ce.0` 做为基础镜像，并且使用上游版本标签 `v10.4.0` 作为对比的基础标签版本，也就是对应于基础镜像版本的标签，然后使用汉化分支 `8-15-stable-zh` 进行对比，生成汉化补丁，由此构建一个名为 `twang2218/gitlab-ce-zh:8-15-stable-zh` 的镜像。
...
这表明将使用 `gitlab/gitlab-ce:10.4.0-ce.0` 镜像为基础镜像，以 `v10.4.0` 为基础对比版本，以 `v10.4.0-zh` 为汉化版本进行对比生成汉化补丁，并构建一个名为 `twang2218/gitlab-ce-zh:10.4.0` 的镜像。
...
这将会以命令 `docker run -d -P twang2218/gitlab-ce-zh:10.4.0` 来运行镜像。这里使用的是 `-P`，因此会随机映射端口。方便测试环境测试，避免和其它端口冲突。
