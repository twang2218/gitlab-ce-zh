...
- [`10.3`, `10.3.0`, `latest` (*10.3/Dockerfile*)](https://github.com/gitlab-ce-zh/gitlab-ce-zh/blob/master/10.3/Dockerfile)
...
```bash
docker run -d -p 3000:80 gitclub/gitlab-ce-zh:10.3.0
```
...
services:
    gitlab:
      image: 'gitclub/gitlab-ce-zh:10.3.0'
...
这表明将使用 `gitlab/gitlab-ce:10.3.0-ce.0` 做为基础镜像，并且使用上游版本标签 `v10.3.0` 作为对比的基础标签版本，也就是对应于基础镜像版本的标签，然后使用汉化分支 `8-15-stable-zh` 进行对比，生成汉化补丁，由此构建一个名为 `gitclub/gitlab-ce-zh:8-15-stable-zh` 的镜像。
...
这表明将使用 `gitlab/gitlab-ce:10.3.0-ce.0` 镜像为基础镜像，以 `v10.3.0` 为基础对比版本，以 `v10.3.0-zh` 为汉化版本进行对比生成汉化补丁，并构建一个名为 `gitclub/gitlab-ce-zh:10.3.0` 的镜像。
...
这将会以命令 `docker run -d -P gitlab-ce-zh/gitlab-ce-zh:10.3.0` 来运行镜像。这里使用的是 `-P`，因此会随机映射端口。方便测试环境测试，避免和其它端口冲突。
